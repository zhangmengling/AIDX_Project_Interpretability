...
class Local_Perturbation(object):
    ...
    def __call__(self, x):
        ...
        # print(self.grad, feed_dict={self.x:np.array([x])})
        ...
        #if gradient all zero
        if np.zeros(self.input_shape).tolist() == ind_grad[0].tolist() and np.zeros(self.input_shape).tolist() == \
                n_ind_grad[0].tolist():
            probs = 1.0 / (self.input_shape-1) * np.ones(self.input_shape)  #if gradient is all 0, generate probabilities 1/shape for all attributes
            probs[self.sens - 1] = 0
        else:
            # nomalize the reciprocal of gradients (prefer the low impactful feature)
            # choose the attribute with lowest contribution (attribute with highest probability)
            grad_sum = 1.0 / (abs(ind_grad[0]) + abs(n_ind_grad[0]))
            grad_sum[self.sens - 1] = 0
            probs = grad_sum / np.sum(grad_sum)
        probs = probs/probs.sum()

        # randomly choose the feature for local perturbation (based on probability)
        index = np.random.choice(xrange(self.input_shape) , p=probs)
...
def dnn_fair_testing(dataset, sensitive_param, model_path, cluster_num, max_global, max_local, max_iter):
    ...
    for num in range(len(inputs)):
        ...
        print("-->index/sample:", index, sample)

        # start global perturbation
        print("-->global perturbation1")
        for iter in range(max_iter+1):
            ...
            prob = probs[label]  # prediction class
            print("-->label/prob", label, prob)
            max_diff = 0
            n_value = -1
            ...
            for i in range(census.input_bounds[sensitive_param-1][0], census.input_bounds[sensitive_param-1][1] + 1):
                if i != sample[0][sensitive_param-1]:
                    ...
                    if label != n_label:
                        ...
                        print("-->prob-n_prob", prob, n_prob, prob - n_prob)
            ...
            if label != n_label and (tuple(temp) not in global_disc_inputs) and (tuple(temp) not in local_disc_inputs):
                print("-->labels", label, n_label)
                print("-->sampels:", sample, n_sample)
            ...
            if np.zeros(data_config[dataset].params).tolist() == s_grad[0].tolist():
                ...
                g_diff = np.array(s_grad[0] == n_grad[0], dtype=float)  # same gradient output 1, different gradient output 0
            ...
            print("-->sample[0]:", sample[0], sample)
