...
class EHR_RNN(EHREmbeddings):
    ...
    def forward(self, input, x_lens, mtd):
        ...
        if self.bi==2:
            ...
            output = self.sigmoid(self.out(output.data[:,-1,:]))
        return output.squeeze()
    
    def get_predict_trace(self, input, x_lens, mtd):
        x_in  = self.EmbedPatient_MB(input, mtd) 
        if self.packPadMode: 
            x_inp = nn.utils.rnn.pack_padded_sequence(x_in,x_lens,batch_first=True)   
            output, hidden = self.rnn_c(x_inp)#,h_0) 
            ...
            output, hidden = self.rnn_c(x_in)#,h_0)
        predict_trace = self.sigmoid(self.out(output))
        return [hn_step.cpu().detach().numpy().tolist() for hn_step in output.squeeze(dim=0)], predict_trace.squeeze(dim=0)
    
        # PackedSequence is used to support batch training
        # output, (hn, cn) = self.forward(input, x_lens, mtd)
